...
public class Reservation implements Serializable
{
  ...
  public boolean doesOverlap(Date date, Time time)
  {
   ...
   long neededTime = calendar.getTimeInMillis();
   
   Calendar exist = Calendar.getInstance();
   Date ex = this.getDate();
   Time exx = this.getTime();
   exist.set(ex.getYear(), ex.getMonth(), ex.getDate(), exx.getHours(), exx.getMinutes(),0);
   
   long existingTime = exist.getTimeInMillis();
   
   long difference = existingTime - neededTime;
   
   if(difference <  7200000 && difference > -7200000)
   {
     return true;
   }
   
   if(neededTime == existingTime) {
     return true;
   }
